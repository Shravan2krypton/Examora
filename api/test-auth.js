...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      return res.json({
        status: '✅ Authentication Test - GET',
        database: {
          connected: true,
    ...
    if (req.method === 'POST') {
      ...
      const { email, password, test_type = 'login' } = req.body;
      ...
      console.log(`🔍 Testing ${test_type} for:`, email);
      ...
      return res.json({
        ...
        next_steps: {
          frontend_login: 'Use this token for frontend authentication',
          api_access: 'Include this token in Authorization header',
          role_based_routing: `Redirect to /${user.role} dashboard`
        }
      });
    }
