...
import { Trophy, Crown, Medal, Star, TrendingUp, Users, Award, Filter, Download } from "lucide-react";
...
export function AdminLeaderboard() {
      ...
      try {
        ...
        if (result.success) {
          ...
          const mockLeaderboard = [
            { rank: 1, studentName: "Alice Johnson", score: 95, submissionTime: new Date(), examsAttempted: 8, averageScore: 94 },
            { rank: 2, studentName: "Bob Smith", score: 92, submissionTime: new Date(), examsAttempted: 7, averageScore: 91 },
            { rank: 3, studentName: "Carol Davis", score: 88, submissionTime: new Date(), examsAttempted: 6, averageScore: 87 },
            { rank: 4, studentName: "David Wilson", score: 85, submissionTime: new Date(), examsAttempted: 5, averageScore: 83 },
            { rank: 5, studentName: "Emma Brown", score: 82, submissionTime: new Date(), examsAttempted: 5, averageScore: 80 },
            { rank: 6, studentName: "Frank Miller", score: 79, submissionTime: new Date(), examsAttempted: 4, averageScore: 78 },
            { rank: 7, studentName: "Grace Lee", score: 76, submissionTime: new Date(), examsAttempted: 4, averageScore: 75 },
            { rank: 8, studentName: "Henry Taylor", score: 73, submissionTime: new Date(), examsAttempted: 3, averageScore: 72 },
            { rank: 9, studentName: "Ivy Chen", score: 70, submissionTime: new Date(), examsAttempted: 3, averageScore: 68 },
            { rank: 10, studentName: "Jack Wilson", score: 67, submissionTime: new Date(), examsAttempted: 2, averageScore: 65 },
          ];
  ...
  const handleExport = () => {
    // Create CSV data
    const csvContent = [
      ["Rank", "Student Name", "Score", "Exams Attempted", "Average Score"],
      ...displayLeaderboard.map(entry => [
        entry.rank,
        entry.studentName,
        entry.score,
        entry.examsAttempted || 0,
    ...
    return (
      <div className="glass-panel p-8 rounded-3xl border border-white/20">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-foreground">Student Leaderboard</h3>
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="text-center p-3 bg-foreground/5 rounded-xl">
          <div className="text-2xl font-bold text-gradient">{leaderboard.length}</div>
          <div className="text-xs text-foreground/50">Total Students</div>
        </div>
        <div className="text-center p-3 bg-foreground/5 rounded-xl">
          <div className="text-2xl font-bold text-gradient">{leaderboard[0]?.score || 0}</div>
          <div className="text-xs text-foreground/50">Top Score</div>
        </div>
        <div className="text-center p-3 bg-foreground/5 rounded-xl">
          <div className="text-2xl font-bold text-gradient">
            {Math.round(leaderboard.reduce((acc, entry) => acc + (entry.averageScore || 0), 0) / leaderboard.length)}
          </div>
